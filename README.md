...
## File Description
| File Name                        | Purpose | Overall Role in Project |
|----------------------------------|---------|-------------------------|
| REPORTE_FINAL_ROBOTICA_V04.pdf   | Academic report documenting the development of a real-time lip movement detection and command execution system integrating speech recognition and LLMs. | Provides the theoretical background, system architecture, experimental methodology, and evaluation results of the full robotics + AI integration system. |
| Final_Proyect_55.py              | Implementation of the full real-time system described in the report. | Contains the complete executable pipeline that integrates vision, audio processing, LLM classification, and drone command logic in a real-time environment. |
